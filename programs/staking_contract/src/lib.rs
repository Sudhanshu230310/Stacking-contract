...
declare_id!("AZuXHBmsM91NMd2Ws6ovfknB32x2VQnmsKQ66J68nFHn");
...
pub mod staking_contract {
    ...
    pub fn unstake(ctx: Context<Unstake>, amount: u64) -> Result<()> {
        ...
        // Transfer SOL from PDA back to user (Manual lamport adjustment for PDAs)
        **ctx.accounts.pda_account.to_account_info().try_borrow_mut_lamports()? = ctx.accounts.pda_account.to_account_info().lamports()
            .checked_sub(amount)
            .ok_or(StakeError::Underflow)?;
        **ctx.accounts.user.to_account_info().try_borrow_mut_lamports()? = ctx.accounts.user.to_account_info().lamports()
            .checked_add(amount)
            .ok_or(StakeError::Overflow)?;
